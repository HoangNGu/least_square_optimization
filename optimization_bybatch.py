...
for participant_number in range(1,5):
    ...
    for loop in range(500):
        ...
        #        Estimation of arousal using alpha beta and c
                ...
                SAGR_error.append(error)
            else:
                SAGR_error.append(math.nan)
        SAGR_error = [x for x in SAGR_error if str(x) != 'nan']
        SAGR_temp_participant.append(np.mean(SAGR_error))
    SAGR_estimate.append(np.mean(SAGR_temp_participant))

print(np.mean(SAGR_estimate))
print(np.std(SAGR_estimate))
        

#Keras DNN - RMSE
#EDA RMSE instead of SAGR

#for SAGR
#A in [1,7] for optimisation
#A estimated is decimal
#scaled from -3 to 3 for SAGR
#take mean for all participant + STD
    #Run 1
#0.480972413334
#0.0425205796608
