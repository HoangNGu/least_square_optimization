...
for i in range(numberOfDataSet):
    for j in range(len(phasicNBpeakList[i])):
        if phasicNBpeakList[i][j] == 0:
            index_del[i].append(j)
            
        
for i in range(numberOfDataSet):
    for index_l in sorted(index_del[i], reverse=True):
        del phasicNBpeakList[i][index_l]
...
amplitude_shuffled.append(np.array(phasicNBpeakList[participant_number-1]))
...
for m in range(500):
    ...
    np.take(phasicNBpeakList[participant_number-1],perm,axis=0,out=amplitude_shuffled[0])
