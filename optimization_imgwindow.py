...
index = [i for i in range(len(meanarousal_window))]
#TEST: DELETE ELEMENT EQUAL TO 0
index_del = [[] for i in range(numberOfDataSet)]
for i in range(numberOfDataSet):
    for j in range(len(phasicPeakAmplitude_20imgs[i])):
        if phasicPeakAmplitude_20imgs[i][j] == 0:
            index_del[i].append(j)
            
        
for i in range(numberOfDataSet):
    for index_l in sorted(index_del[i], reverse=True):
        del phasicPeakAmplitude_20imgs[i][index_l]

for index_l in sorted(index_del[participant_number-1], reverse=True):
        del meanarousal_window[index_l]
        del index[index_l]


#Permutation to shuffle
amplitude_shuffled, arousal_mean_shuffled, index_shuffled = [], [], []
amplitude_shuffled.append(np.array(phasicPeakAmplitude_20imgs[participant_number-1]))
arousal_mean_shuffled = np.array(meanarousal_window)
...
global_gamma = []
for m in range(500):
    perm = np.random.permutation(len(meanarousal_window))
    np.take(phasicPeakAmplitude_20imgs[participant_number-1],perm,axis=0,out=amplitude_shuffled[0])
    ...
    for n in range(1):
        ...
        def eq( p , arousal, index, peaksAvgMagn):
            ...
            for i in range(len(index)):
                ind = index[i]
                f = log(arousal[i])+a*(ind**2)+b*(ind**1)+g-log(peaksAvgMagn[i])
                err.append(f)
            return (err) 
...
for i in range(len(half2_arousal)):
    amp_est_temp = half2_arousal[i]*exp(alpha_mean*(half2_index[i]**2)+beta_mean*(half2_index[i]**1)+gamma_mean)
    amplitude_Estimate.append(amp_est_temp)

estimate_error = []
for i in range(len(amplitude_Estimate)):
    estimate_error.append((halves2_amplitude[0][i] - amplitude_Estimate[i])/halves2_amplitude[0][i])


#%%

plt.figure()
plt.title("Dot size proportional to average amplitude per picture - Real+Estimate")
plt.xlabel('timebin')
plt.ylabel('Arousal')
#x = pla.arange(1., len(half2_arousal)+1.)
colors = ['r','g','b']
#    trr = phasicPeakAmplitude[i]
#    normalized = (trr-min(trr))/(max(trr)-min(trr))
s = [(k)*1000 for k in amplitude_Estimate]
s2 = [(k)*1000 for k in halves2_amplitude[0]]
plt.scatter(half2_index, half2_arousal, s, marker= 'o', alpha=0.5, cmap = 'seismic',edgecolors='black')
plt.scatter(half2_index, half2_arousal, s2, marker= 'o', alpha=0.5, cmap = 'seismic',edgecolors='black')
plt.legend()
plt.show()

plt.figure()
plt.title("Dot size proportional to average amplitude per picture - Real")
plt.xlabel('timebin')
plt.ylabel('Arousal')
#x = pla.arange(1., len(half2_arousal)+1.)
colors = ['r','g','b']
...
#    normalized = (trr-min(trr))/(max(trr)-min(trr))
s2 = [(k)*1000 for k in halves2_amplitude[0]]
plt.scatter(half2_index, half2_arousal, s2,c = s2, marker= 'o', alpha=0.5, cmap = 'seismic',edgecolors='black')
plt.show()

plt.figure()
plt.title("Dot size proportional to average amplitude per picture - Estimate")
plt.xlabel('timebin')
plt.ylabel('Arousal')
#x = pla.arange(1., len(half2_arousal)+1.)
colors = ['r','g','b']
...
#    normalized = (trr-min(trr))/(max(trr)-min(trr))
s = [(k)*1000 for k in amplitude_Estimate]
plt.scatter(half2_index, half2_arousal, s,c = s, marker= 'o', alpha=0.5, cmap = 'seismic',edgecolors='black')
plt.show()

#exclude 0
#use index should be ok (try use t in sec)
#std
#2.624621396810935e-12
#5.2086302258528301e-10
#1.6369371809746211e-08
