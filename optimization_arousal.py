...
global_beta = []
global_gamma = []
p_global = [[] for i in range(poly_order + 1)]
#global_theta = []
...
#    Permutation 
    ...
    ps = [[] for i in range(poly_order + 1)]
    ...
    for n in range(1):
        ...
        def eq( p0 , arousal, index, peaksAvgMagn):
            p = p0
            err = []
...
p_std = []
for i in range(poly_order + 1): 
    p_std.append(np.std(p_global[i]))
...
for i in range(len(half2_arousal)):
    arousal_est_temp = 0
    for j in range(poly_order + 1):
        arousal_est_temp = arousal_est_temp + p_mean[j]*(half2_index[i]**j)
...
plt.figure()
plt.title("Estimate_Arousal")
plt.xlabel('image index')
plt.ylabel('Arousal')
plt.plot(Arousal_Estimate)
plt.show()

plt.figure()
ax = plt.gca()
plt.title("Arousal half 2 GT")
plt.xlabel('image index')
plt.ylabel('Arousal')
plt.plot(half2_arousal)
plt.show()
